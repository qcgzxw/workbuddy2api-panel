// metrics.go 请求统计聚合（/v1/stats 数据源，吸收上游 733d348 + 5009a1f）。
//
// 同步账（@1.11.3-panel 决策）：**明确跳过**同链的 cmd/stats CLI 三条——ffeaad1
// （/v1/stats 终端入口）、6925fb2（倍率列 + -sort credits）、4cd79cd（其帧时间戳
// 测试修复）。理由：本仓库没有 cmd/stats（面板 internal/panel 已覆盖同一数据源的
// 可读展示），再引一个终端视图只会多一处需要跟着改的消费者。若将来要补，从
// ffeaad1 整条开始搬，勿只搬倍率列（列渲染依赖入口的表格骨架）。
//
// 设计要点（与上游同口径，便于下次同步对齐）：
...
//   - **倍率透出只读**：credits 是展示字段（非观测值），不进聚合路径，出口处从模型
//     目录**只读快照**合入（enrichCredits），缓存冷就冷，绝不为此发起上游调用。
package server
...
//
// total 为端到端耗时（TTFB 与生成吞吐的分母口径均由此派生）。模型名为空时归入
// "-" 键（仍计入 total，不丢弃观测）。
...
// fillStatFromReader 把流式路径末帧 usage 的观测填进 chatStat（与成本账本同源同口径，
// 都读末帧 usage，避免二次解析）。
//
// hasUsage 取「三段 token 任一有观测」：上游 usage 形态不完全一致（有的只回
// completion_tokens），任一字段到位即算拿到观测——与"缺失≠0"的纪律一致（真的没
// 任何字段时保持 false，不把零值伪装成观测）。
func fillStatFromReader(st *chatStat, r *chatStatsReader) {
	d := r.Usage()
	st.hasUsage = d.HasPromptTokens || d.HasCompletionTokens || d.HasTotalTokens
...
func fillStatFromUsage(st *chatStat, resp map[string]any) {
	...
	_, hasPrompt := usageInt(u, "prompt_tokens")
	_, hasCompletion := usageInt(u, "completion_tokens")
	_, hasTotal := usageInt(u, "total_tokens")
	st.hasUsage = hasPrompt || hasCompletion || hasTotal
...
// 调用（网关只加工已有数据）。倍率是展示字段而非观测值，故不进 recordChatMetric
// 聚合路径，快照出口统一合入。
//
