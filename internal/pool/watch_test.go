...
func TestDirFingerprintDetectsChange(t *testing.T) {
	...
	// 删除 → 指纹回到空串（调用方据 ok=true + 值变化识别"全删"）
	if err := os.Remove(filepath.Join(dir, "workbuddy-u1.json")); err != nil {
		t.Fatal(err)
	}
	if fp, ok := dirFingerprint(dir); !ok || fp != "" {
		t.Errorf("清空目录后应回到空串指纹且 ok=true，得到 %q ok=%v", fp, ok)
	}
}
...
// TestStartAuthDirWatchFiresOnAdd 轮询循环本体：目录新增凭证后，无需任何显式调用即自动进池。
//
// 与 TestReloadAuthDirAddsAccount 的分工：那个测 reloadAuthDir 的对齐逻辑，本用例测
// **ticker 驱动的触发链**（指纹变化 → 重新加载），即监听真正"活着"。
func TestStartAuthDirWatchFiresOnAdd(t *testing.T) {
	dir := t.TempDir()
	writeAuthFile(t, dir, "u1", "一号")
	...
	for len(p.AvailableUIDs()) != 2 {
		if time.Now().After(deadline) {
			t.Fatalf("新增凭证未被热加载，可用账号数=%d want 2", len(p.AvailableUIDs()))
		}
		time.Sleep(5 * time.Millisecond)
	}

	// 停止后目录再变也不应生效（避免测试结束后 goroutine 继续动池）
	stop()
	if err := os.Remove(filepath.Join(dir, "workbuddy-u2.json")); err != nil {
		t.Fatal(err)
	}
	time.Sleep(50 * time.Millisecond)
	if n := len(p.AvailableUIDs()); n != 2 {
		t.Errorf("停止监听后仍发生同步：可用账号数=%d want 2", n)
	}
}
...
func TestReloadAuthDirPreservesState(t *testing.T) {
	...
	if after.CoolKind != before.CoolKind {
		t.Errorf("cool_kind 被改变: %q → %q", after.CoolKind, before.CoolKind)
	}
