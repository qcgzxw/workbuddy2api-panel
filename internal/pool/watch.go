...
// 凭证文件不会进池——表现为面板/控制台显示"已添加"但账号"未加载"，必须手动重启才生效。
// 注意覆盖面：面板登录路径自带 pool.Add + Revive（internal/panel/login.go），本监听补的是
// **目录侧**写入（login.sh 落盘、手工拷文件、外部脚本生成凭证）这条路径。
//
// 实现选择——轮询而非 fsnotify：
...
// credits/cooling/成本账本，故重复调用不会重置任何运行状态（见 watch_test.go 的
// TestReloadAuthDirPreservesState，该契约一旦被破坏，每次加号都会清空全池冷却）。
package pool
...
func (p *Pool) StartAuthDirWatch(dir string) (stop func()) {
	...
	log.Printf("[watch] auths 目录监听已启用（每 %s 检查一次，新增账号自动进池，无需重启）", watchInterval)

	// interval 在启动 goroutine 前取值：watchInterval 是包级可变变量（测试会改短），
	// 让监听 goroutine 去读它等于跨 goroutine 读一个可写变量（-race 下测试会报竞争），
	// 与 persist.go 的 startFlusher「启动前同步取值」同一口径。
...
// 故半写入的临时文件（login.sh 类脚本用 tempfile + os.replace 原子替换）不会造成误判。
func (p *Pool) reloadAuthDir(dir string) {
	...
	if after != before {
		log.Printf("[watch] auths 目录变化：可用账号数 %d → %d（已热加载，无需重启）", before, after)
	} else {
		// 数量不变但内容变了（如凭证刷新、文件改名）：仍要落一条，便于对账。
		log.Printf("[watch] auths 目录变化：可用账号数保持 %d（已热加载凭证更新）", after)
	}
}

// dirFingerprint 生成目录内容指纹：文件名 + 修改时间 + 大小，排序后拼接。
//
// 为什么不用纯文件名：凭证刷新（同名覆盖 uid 文件）时文件名不变，
